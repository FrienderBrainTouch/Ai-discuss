...
public class TeacherServiceTest {
    ...
    void signup_success(){
        //given
        String email = "testTeacher@test.com";
        String password = "password123";
        Teacher teacher = Teacher.createTeacher(email, password);
        when(teacherRepository.save(any(Teacher.class))).thenReturn(teacher);

        //when
        Teacher savedTeacher = teacherService.signup(email, password);

        //then
        assertThat(savedTeacher.getUserEmail()).isEqualTo(email);
    ...
    void signup_fail_with_duplicate_email() {
        ...
        Teacher existingTeacher = Teacher.createTeacher(email, "someOtherPassword");

        // 2. teacherRepository.findByEmail(email)이 호출되면,
        //    null이 아닌 위에서 만든 '기존 사용자' 객체를 Optional에 담아 반환하도록 설정합니다.
        when(teacherRepository.findByUserEmail(email)).thenReturn(Optional.of(existingTeacher));
        ...
        IllegalStateException exception = assertThrows(IllegalStateException.class, () -> {
            teacherService.signup(email, password);
        });
